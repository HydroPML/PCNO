...
model_con = FNO2d(num_channels=num_channels, initial_step=initial_step, modes1=modes, modes2=modes, width=width,
                  grid_type=grid_type).cuda()
path_model_con = '/pre_trainined/Models/2D_Flood/PCNO/Australia/288/model.pt'
...
## Datasets for T=24h
path_train = '/Path_to/Datasets/Australia/288/train'
path_val = '/Path_to/Datasets/Australia/288/valid'
path_test = '/Path_to/Datasets/Australia/288/test'
## Datasets for T=12h
# path_train = '/Path_to/Datasets/Australia/144/train'
# path_val = '/Path_to/Datasets/Australia/144/valid'
# path_test = '/Path_to/Datasets/Australia/144/test'
...
valid_data = pde_data(path_val, train=False, strategy=args.strategy, T_in=T_in, T_out=T, rain=args.rain, global_max=global_max)
nvalid = len(valid_data)
test_data = pde_data(path_test, train=False, strategy=args.strategy, T_in=T_in, T_out=T, rain=args.rain, global_max=global_max)
...
train_loader1 = torch.utils.data.DataLoader(train_data, batch_size=20, shuffle=True)
...
def get_eval_pred(model, model_con, x, y_c, strategy, T, times):
...
for ep in range(epochs):
    ...
    with torch.no_grad():
        ...
        for xx, yy in valid_loader:
            ...
            pred = get_eval_pred(model=model, model_con=model_con, x=x, y_c=yy_c, strategy=args.strategy, T=T, times=eval_times)
...
with torch.no_grad():
    ...
    for xx, yy in test_loader:
        ...
        pred = get_eval_pred(model=model, model_con=model_con, x=x, y_c=yy_c, strategy=args.strategy, T=T, times=[])
        pred = pred * global_max
        pred = torch.expm1(pred)
