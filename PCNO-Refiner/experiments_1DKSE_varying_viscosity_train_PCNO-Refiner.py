...
parser.add_argument("--results_path", type=str, default="/Path_to/results/KDE/PCNO-Refiner/", help="path to store results")
parser.add_argument("--suffix", type=str, default="seed1", help="suffix to add to the results path")
parser.add_argument("--txt_suffix", type=str, default="1D_KDE_PCNO-Refiner_sampling_seed1", help="suffix to add to the results txt")
...
if args.model_type in ["Unet"]:
    model = Unet(n_input_scalar_components=args.n_scalar_components, n_input_vector_components=args.n_vector_components,
         ...
         time_future=args.time_future, hidden_channels=32, activation=args.activation,
...
for ep in range(epochs):
    ...
    for xx, yy, cond in tqdm(train_loader, disable=not args.verbose):
        ...
        x0 = (x * 0.5 + 0.5)
        with torch.no_grad():
            im = model_con(x0, z=cond)
            # im = (im - 0.5) / 0.5
        im_c = torch.cat((im, x0), dim=1)
        ...
        sigmas = karras_schedule(
            num_timesteps, sigma_min, sigma_max, rho, y.device
        )
        noise = torch.randn_like(y)

        timesteps = lognormal_timestep_distribution(
            y.shape[0], sigmas, lognormal_mean, lognormal_std
        )

        current_sigmas = sigmas[timesteps]
        next_sigmas = sigmas[timesteps + 1]

        next_noisy_y = y + pad_dims_like(next_sigmas, y) * noise
        ...
        with torch.no_grad():
            current_noisy_y = y + pad_dims_like(current_sigmas, y) * noise
