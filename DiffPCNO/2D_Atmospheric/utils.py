...
class grid(torch.nn.Module):
    ...
    def threeD_grid(self, x):
        ...
        gridx = torch.linspace(0, 2 * torch.pi, size_x).reshape(1, size_x, 1, 1, 1).repeat([batchsize, 1, size_y, size_z, 1])
        gridy = torch.linspace(0, 2 * torch.pi, size_y).reshape(1, 1, size_y, 1, 1).repeat([batchsize, size_x, 1, size_z, 1])
