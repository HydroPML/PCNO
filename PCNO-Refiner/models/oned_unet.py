...
class Unet(nn.Module):
    ...
    def __init__(
        ...
        insize = time_history * (self.n_input_scalar_components + self.n_input_vector_components * 2) + 1
    ...
    def forward(self, x: torch.Tensor, time: torch.Tensor = None, x_self_cond: torch.Tensor = None, z: torch.Tensor = None):
        x_ori = x
        ...
        # emb = emb.to(x.device)
        # self.time_embed = self.time_embed.to(x.device)
        # print('time', time.device)
        ...
        if z is not None:
            ...
            if self.param_conditioning.startswith("scalar"):
                ...
                for i in range(z.shape[-1]):
                    # self.pde_emb[i] = self.pde_emb[i].to(x.device)
        ...
        x = x.unsqueeze(-2)
        # print('x_final0', x.shape)
        # t = t - self.eps
        # c_skip_t = 0.5 ** 2 / (t.pow(2) + 0.5 ** 2)
        # c_out_t = 0.5 * t / ((t + self.eps).pow(2) + 0.5 ** 2).pow(0.5)
        # # print('x_ori',x_ori.shape)
        # x = c_skip_t[:, None, None, None] * x_ori + c_out_t[:, None, None, None] * x
    ...
    def sample(self, x, ts: List[float], x_self_cond, z):
        ...
        x_out = self(x, torch.tensor(ts[0]).cuda().unsqueeze(0), x_self_cond, z)
        ...
        for sigma in ts[1:]:
            ...
            x_out = self(x, sigma, x_self_cond, z)
            x = self.output_scale(x, x_out, sigma, sigma_data, sigma_min)
