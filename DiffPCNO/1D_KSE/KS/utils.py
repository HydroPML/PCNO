...
class pde_data(torch.utils.data.Dataset):
    def __init__(self, data, T_in, T_out=None, train=True, strategy="markov", std=0.0):
        ...
        self.data_orignal = data
        self.time_step = 4
        self.T_out = int((T_in + T_out)/self.time_step - T_in)
        # self.T_out = T_out
        self.T_in = T_in
        self.nt = self.T_in + self.T_out
        ...
        for i in range(self.time_step):
            data_i = self.data_orignal["u"][:, i::self.time_step, :, :]
            data_u.append(data_i)
        data["u"] = torch.cat(data_u, dim=0)
        data_t = []
        for i in range(self.time_step):
            data_i = self.data_orignal["dt"]
            data_t.append(data_i)
        data["dt"] = torch.cat(data_t, dim=0)
        data_x = []
        for i in range(self.time_step):
            data_i = self.data_orignal["dx"]
            data_x.append(data_i)
        data["dx"] = torch.cat(data_x, dim=0)
        # data_v = []
        # for i in range(self.time_step):
        #     data_i = self.data_orignal["v"]
        #     data_v.append(data_i)
        # data["v"] = torch.cat(data_v, dim=0)
        self.data = data

    def __len__(self):
        if self.train:
            if self.markov:
                return len(self.data["u"]) * (self.nt - 1)
            if self.teacher_forcing:
                return len(self.data["u"]) * (self.nt - self.T_in)
    ...
    def __getitem__(self, idx):
        ...
        # if self.train and self.allow_shuffle:
        #     np.random.shuffle(idxs)

        if not self.train or not (self.markov or self.teacher_forcing): # full target: return all future steps
            cond = [self.data["dt"][idx], self.data["dx"][idx]]
            if "v" in self.data:
                cond.append(self.data["v"][idx])
            if self.usegrid:
            ...
            #     else:
            #         start_idx = 0
            #     pde = pde[start_idx:: self.time_step]
        ...
        y = pde[t_idx:(t_idx+1), ...]
        cond = [self.data["dt"][pde_idx], self.data["dx"][pde_idx]]
        if "v" in self.data:
            cond.append(self.data["v"][pde_idx])
        if self.usegrid:
            grid = np.linspace(0, 1, u.shape[-1])
        else:
            grid = None
