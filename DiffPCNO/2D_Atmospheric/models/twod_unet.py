...
class Unet(nn.Module):
    ...
    def __init__(
        ...
        self.noise_level_channels = 256
        self.noise_level_scale = 0.02

        insize = time_history * (self.n_input_scalar_components + self.n_input_vector_components * 2) + 9
        print('insize',insize)
        ...
        out_channels = time_future * (self.n_output_scalar_components + self.n_output_vector_components * 2) + 2
        if use1x1:
        ...
        self.noise_level_embedding = NoiseLevelEmbedding(self.noise_level_channels, self.noise_level_scale)

    def forward(self, x, time, x_self_cond=None, z=None):
        # print('x',x.shape)
        ...
        if time is not None:
            emb = self.noise_level_embedding(time)
    ...
    def output_scale(self, x, x_out, sigma, sigma_data, sigma_min):
    ...
    @torch.no_grad()
    def sample(self, x, ts: List[float], x_self_cond):
        # print('torch.tensor(ts[0]).cuda()', torch.tensor(ts[0]).unsqueeze(0).repeat(x.shape[0]).shape)
        # print('x',x.shape)
        x_out = self(x, torch.tensor(ts[0]).cuda().unsqueeze(0).repeat(x.shape[0]), x_self_cond)
        sigma = torch.full((x.shape[0],), ts[0], dtype=x.dtype, device=x.device)
        sigma_data = 0.5
        sigma_min = 0.002
        x = self.output_scale(x, x_out, sigma, sigma_data, sigma_min)
        ...
        for sigma in ts[1:]:
            ...
            x = x + math.sqrt(sigma ** 2 - self.eps ** 2) * zz
            x_out = self(x, sigma.repeat(x.shape[0]), x_self_cond)
            x = self.output_scale(x, x_out, sigma, sigma_data, sigma_min)
...
class FourierUnet(nn.Module):
    ...
    def __init__(
        ...
        insize = time_history * (self.n_input_scalar_components + self.n_input_vector_components * 2) + 3
        print('insize', insize)
        n_channels = hidden_channels
        self.activation: nn.Module = ACTIVATION_REGISTRY.get(activation, None)
        if self.activation is None:
