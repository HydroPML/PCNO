...
from models.PCNO3D import PCNO2d, PCNO3d
...
from utils_sw import pde_data, LpLoss, eq_check_rt, eq_check_rf, div_free_2d_check, div_free_3d_check, count_params, div_free_3d_check_speedyweather
...
parser.add_argument("--results_path", type=str, default="/Path_to/PCNO3D/results/climate/", help="path to store results")
parser.add_argument("--suffix", type=str, default="seed1", help="suffix to add to the results path")
parser.add_argument("--txt_suffix", type=str, default="Climate_PCNO3D_seed1", help="suffix to add to the results txt")
...
parser.add_argument("--model_type", type=str, default='PCNO3d')
...
parser.add_argument("--early_stopping", type=int, default=500, help="stop if validation error does not improve for successive epochs")
...
threeD = args.model_type in ["PCNO3d", "FNO3d_aug",
                             "GCNN3d_p4", "GCNN3d_p4m",
                             "GFNO3d_p4", "GFNO3d_p4m",
                             "radialNO3d_p4", "radialNO3d_p4m",
                             "Unet_Rot_3D"]
...
elif args.model_type in ["PCNO3d"]:
    modes3 = time_modes if time_modes else modes
    model = PCNO3d(num_channels=num_channels, initial_step=initial_step, modes1=modes, modes2=modes, modes3=modes3,
                  width=width, grid_type=grid_type, time=time, time_pad=args.time_pad).cuda()
...
if ns_zli and ns_zli_read:  # incompressible NS data in FNO paper
    ...
    sin_colat = torch.sin(colat).reshape(1, Sy, 1).repeat([Sx, 1, T])
    # data[..., 1] /= data[..., 2]
    # data[..., 2] /= sin_colat
    # data[..., 0] /= data[..., 2]
else:
...
i_flag_check_div_free_3d = 1
if i_flag_check_div_free_3d:
    if rdb:
        i_div_free = div_free_3d_check(data[..., 0:1], data[..., 1:2], data[..., 2:3])
    elif swe:
        i_div_free = div_free_3d_check_speedyweather(data[6:7, ..., 0:1], data[6:7, ..., 1:2], data[6:7, ..., 2:3], lon, colat)
# print('i_div_free', i_div_free.shape)
print('i_div_free_data', i_div_free.mean())
train = data[:ntrain]
...
for ep in range(epochs):
    ...
    for xx, yy in tqdm(train_loader, disable=not args.verbose):
        ...
        if args.strategy == "recurrent":
            ...
            # i_div_free = div_free_3d_check_speedyweather(im[0:1, ..., 0:1].detach().cpu(),
            #                                              im[0:1, ..., 1:2].detach().cpu(),
            #                                              im[0:1, ..., 2:3].detach().cpu(), lon, colat)
            # # print('i_div_free', i_div_free.shape)
            # print('i_div_free_training', i_div_free.mean())
