...
class GConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, bias=True, first_layer=False, last_layer=False,
        ...
        self.kernel_size_X_H = kernel_size // 2 + 1
        ...
        if first_layer or last_layer:
            ...
            if self.Hermitian:
                self.W = nn.ParameterDict({
                    'y0_modes': torch.nn.Parameter(torch.empty(out_channels, 1, in_channels, 1, self.kernel_size_X_H - 1, self.kernel_size_Y, dtype=dtype)),
                    'yposx_modes': torch.nn.Parameter(torch.empty(out_channels, 1, in_channels, 1, self.kernel_size_Y, self.kernel_size_X_H - 1, dtype=dtype)),
                    '00_modes': torch.nn.Parameter(torch.empty(out_channels, 1, in_channels, 1, 1, self.kernel_size_Y, dtype=torch.float))
                })
            else:
                self.W = nn.ParameterDict({
                    'y0_modes': torch.nn.Parameter(
                        torch.empty(out_channels, 1, in_channels, 1, self.kernel_size_X_H - 1, self.kernel_size_Y, dtype=torch.float)),
                    'yposx_modes': torch.nn.Parameter(
                        torch.empty(out_channels, 1, in_channels, 1, self.kernel_size_Y, self.kernel_size_X_H - 1, dtype=torch.float)),
                    '00_modes': torch.nn.Parameter(torch.empty(out_channels, 1, in_channels, 1, 1, self.kernel_size_Y, dtype=torch.float))
                })
    ...
    def get_weight(self):
        ...
        if self.first_layer or self.last_layer:
            ...
            self.weights = self.W.repeat(1, self.group_size, 1, 1, 1)
            ...
            if self.Hermitian:
                ...
                self.weights = torch.cat(
                    [self.W["y0_modes"], self.W["00_modes"].float(), -self.W["y0_modes"].flip(dims=(-2,))], dim=-2)
        ...
        if self.Hermitian:
            self.weights = self.weights[..., -self.kernel_size_X_H:]
...
class GFNO2d(nn.Module):
    def __init__(self, num_channels, modes, width, initial_step, reflection, grid_type):
        ...
        self.norm = GNorm(self.width, group_size=1)
