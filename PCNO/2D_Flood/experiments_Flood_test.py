...
parser.add_argument("--ntrain", type=int, default=80, help="training sample size")
parser.add_argument("--nvalid", type=int, default=16, help="valid sample size")
parser.add_argument("--ntest", type=int, default=16, help="test sample size")
...
root = '/Pre_trained/Models/2D_Flood/Pakistan'
...
with torch.no_grad():
    for xx, yy in test_loader:
        ...
        # print('pred', pred.max())
        test_l2 += lploss(pred.reshape(len(pred), -1, num_channels_y),
                          yy_g.reshape(len(yy_g), -1, num_channels_y)).item()
        print('loss_batch', lploss(pred.reshape(len(pred), -1, num_channels_y),yy_g.reshape(len(yy_g), -1, num_channels_y)).item())
