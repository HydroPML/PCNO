...
def fft_mesh_2d(nx, ny, diam, device=None):
    # diam = 2 * np.pi
...
def fft_expand_dims3d(fft_mesh, batch_size):
    kx, ky, kz = fft_mesh
    kx, ky, kz = [repeat(z, "x y z -> b x y z 1", b=batch_size) for z in [kx, ky, kz]]
    return kx, ky, kz

def fft_mesh_3d(nx, ny, nt, diam, device=None):
    # diam = 2 * np.pi
    # kx = fft.fftfreq(nx, d=diam / nx)
    # ky = fft.fftfreq(ny, d=diam / ny)
    # kz = fft.fftfreq(nt, d=1.0 / nt)
    gridx = torch.arange(0, 2.0 * torch.pi, 2.0 * torch.pi / nx)
    gridy = torch.linspace(1.875, 178.125, ny) / 180 * torch.pi
    ...
    # print('kx', kx.shape)
    return kx.cuda(), ky.cuda(), kz.cuda()

def spectral_laplacian_3d(fft_mesh, device=None):
    kx, ky, kz = fft_mesh
    lap = -(kx ** 2 + ky ** 2 + kz ** 2)
    lap[..., 0, 0] = 1
    return lap.to(device)

def spectral_div_3d(vhat, rfft_mesh):
    r"""
    Computes the 2D divergence in the Fourier basis.
    """
    uhat, vhat, what = vhat
    ...
    # print(kx.shape)
    # print(ky.shape)
    return 1j * (uhat * kx + vhat * ky + what * kz)


def spectral_grad_3d(vhat, rfft_mesh):
    kx, ky, kz = rfft_mesh
    kx = kx[..., 0]
    ky = ky[..., 0]
    kz = kz[..., 0]
    return 1j * kx * vhat, 1j * ky * vhat, 1j * kz * vhat
