...
class grid(torch.nn.Module):
    ...
    def threeD_grid(self, x):
        ...
        if not self.symmetric:
            ...
            midx = (torch.max(gridx) - torch.min(gridx)) / 2.0
            midy = (torch.max(gridy) - torch.min(gridy)) / 2.0
            # midx = midy = 0.
            gridx = (gridx - midx) ** 2
            gridy = (gridy - midy) ** 2
            grid = torch.cat((gridx + gridy, gridz), dim=-1)
...
class fc(object):
    def __init__(self, fc_d=3, fc_C=25):
        ...
        self.fc_AQ = torch.matmul(fc_A, fc_Q.T)
        self.fc_AlQl = torch.matmul(torch.matmul(torch.matmul(fc_F2, fc_A), fc_Q.T), fc_F1)
...
def div_free_3d_check_speedyweather(hu, hv, h, lon, lat):
    ...
    i_div_free = dhudx + dhvdy + dhdz
    deleteme = dhudx + dhvdy
    ...
    i_div_free_ref = np.gradient(x_ref, dx, dy, dz)[0] + np.gradient(y_ref, dx, dy, dz)[1] + np.gradient(z_ref, dx, dy, dz)[2]
    deleteme_ref = np.gradient(x_ref, dx, dy, dz)[0] + np.gradient(y_ref, dx, dy, dz)[1]
    dhdz_ref = np.gradient(z_ref, dx, dy, dz)[2]
