...
class ElucidatedDiffusion(nn.Module):
    ...
    def preconditioned_network_forward(self, noised_images, sigma, self_cond = None, z = None, clamp = False):
        ...
        padded_sigma = padded_sigma.to(device)
    ...
    @torch.no_grad()
    def sample(self, self_cond, z = None, batch_size = None, num_sample_steps = None, clamp = True, get_sampling=False):
        batch_size = self_cond.shape[0]
        num_sample_steps = default(num_sample_steps, self.num_sample_steps)

        shape = (batch_size, self.channels, 1, self.image_size)
        ...
        for sigma, sigma_next, gamma in sigmas_and_gammas:
            ...
            # print('images_hat', images_hat.shape)

            model_output = self.preconditioned_network_forward(images_hat, sigma_hat, self_cond, z, clamp = clamp)
            ...
            if sigma_next != 0:
                ...
                model_output_next = self.preconditioned_network_forward(images_next, sigma_next, self_cond, z, clamp = clamp)
    ...
    def forward(self, images, self_cond=None, z=None):
        # print(*images.shape)
        batch_size, c, _, h, device, image_size, channels = *images.shape, images.device, self.image_size, self.channels
        # print('device',device)

        assert h == image_size, f'height of image must be {image_size}'
        ...
        padded_sigmas = padded_sigmas.to(device)
        ...
        denoised = self.preconditioned_network_forward(noised_images, sigmas, self_cond, z)
