...
path_model_con = '/pre_trainined/Models/2D_Flood/PCNO/Pakistan/288/model.pt'
...
for ep in range(epochs):
    ...
    with torch.no_grad():
        ...
        for xx, yy in valid_loader:
            ...
            valid_l2 += lploss(pred.reshape(len(pred), -1, num_channels), yy_g.reshape(len(yy_g), -1, num_channels)).item()
...
with torch.no_grad():
    ...
    for xx, yy in test_loader:
        ...
        pred = pred * global_max
        pred = torch.expm1(pred)
        test_l2 += lploss(pred.reshape(len(pred), -1, num_channels), yy_g.reshape(len(yy_g), -1, num_channels)).item()
