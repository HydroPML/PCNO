...
root = '/Path_to_pre_trained/Models/1D_KSE/DiffPCNO'
...
def dataset(path, mode):
    with h5py.File(path, "r") as f:
        ...
        # shift_x = data["dx"].min()
        # scale_x = data["dx"].max() - data["dx"].min()
        # data["dx"] = (data["dx"] - shift_x) / scale_x
        #
        # shift_v = data["v"].min()
        # scale_v = data["v"].max() - data["v"].min()
        # data["v"] = (data["v"] - shift_v) / scale_v
    ...
    for i in range(num_samples):
        ...
        for t in range(T):
            ...
            if t == 0:
                ...
                pred_r = torch.cat((pred_r, im_r), 1)
            if strategy == "markov":
                x0 = im
                # x = x.clamp(0, 1)
            else:
...
with torch.no_grad():
    ...
    for xx, yy, cond in test_loader:
        ...
        # numsamples = preds.shape[0]
        # losses = []
        # for m in range(numsamples):
        #     loss_i = lploss(
        #         preds[m].reshape(len(y), -1, num_channels),
        #         y.reshape(len(y), -1, num_channels)
        #     ).item()
        #     losses.append(loss_i)
        # best_idx = min(range(numsamples), key=lambda i: losses[i])
        # print('best_idx', best_idx)
        # best_loss = losses[best_idx]
        # test_l2 += best_loss
        # pred = preds[best_idx]
        # pred_hm, pred_um, pred_vm = torch.unsqueeze(pred_h, dim=-1), torch.unsqueeze(pred_u, dim=-1), torch.unsqueeze(pred_v, dim=-1)
        # out = torch.cat((pred_hm, pred_um, pred_vm), dim=-1)
        # outm, _ = torch.max(out, dim=3, keepdim=True)
        # outm = torch.rot90(pred_hm, k=-1, dims=[1, 2])
        #
        ...
        # MOIVE
        movie_dir = '/Path_to/Results/KS/DiffPCNO/%s/' % (str(i))
        os.makedirs(movie_dir, exist_ok=True)
        # H
        movie_name = 'H.gif'
...
# torch.save(g_f, 'Path_to/Results/g_f.pt')
# torch.save(p_f, 'Path_to/Results/p_f.pt')
# torch.save(p_std_f, 'Path_to/Results/p_std_f.pt')
# gt_f = g_f.cpu().numpy()
...
# file1 = "Path_to/Results/gt.xlsx"
# file2 = "Path_to/Results/pre.xlsx"
# file3 = "Path_to/Results/gt_ave.xlsx"
# file4 = "Path_to/Results/pre_ave.xlsx"
# file5 = "Path_to/Results/e_ave.xlsx"
# file6 = "Path_to/Results/pre_std.xlsx"
# file7 = "Path_to/Results/pre_std_ave.xlsx"
# #
# # # #
...
# # #
# # #
# df_gs = pd.DataFrame(g_mean, columns=['DiffPCNO-Residual'])
# df_gs.to_excel(file3, index=False)
# df_ps = pd.DataFrame(p_mean, columns=['DiffPCNO-Residual'])
# df_ps.to_excel(file4, index=False)
# df_es = pd.DataFrame(e_mean, columns=['DiffPCNO-Residual'])
# df_es.to_excel(file5, index=False)
# df_pss = pd.DataFrame(p_std_mean, columns=['DiffPCNO-Residual'])
